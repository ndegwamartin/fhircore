...
class SamplePatients {
  ...
  private fun createPatientItem(position: Int, patient: Patient): PatientListViewModel.PatientItem {
    ...
    val ext = patient.extension.singleOrNull {
      it.value.toString().contains("risk")
    }
    val risk = ext?.value?.toString()?:""

    return PatientListViewModel.PatientItem(patient.id, name, gender, dob, html, phone, logicalId, risk)
  }
