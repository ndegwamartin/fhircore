...
@Config(
  ...
  shadows = [QuestionnaireActivityTest.CustomBuilder::class]
)
class QuestionnaireActivityTest : RobolectricTest() {
  ...
  fun setUp() {
    ...
    fhirApplication.setTheme(R.style.AppTheme)
    val questionnaireConfig =
      QuestionnaireConfig(form = "patient-registration", title = "Add Patient", identifier = "1452")
    questionnaireViewModel = spyk(QuestionnaireViewModel(fhirApplication, questionnaireConfig))
    val samplePatientRegisterQuestionnaire =
      AssetUtil.loadAsset<Questionnaire>(
        fileName = REGISTER_QUESTIONNAIRE_ID,
        context = fhirApplication,
        parseResource = true
      )
    ...
    coEvery { fhirEngine.load(Patient::class.java, TEST_PATIENT_1_ID) } returns FakePatient.patient1
