...
import org.smartregister.fhircore.sharedtest.robolectric.FhirApplication
...
@Config(application = FhirApplication::class, shadows = [ShadowNpmPackageProvider::class])
...
class QuestionnaireViewModelTest : RobolectricTest() {
  ...
  @Before
  fun setUp() {
    fhirApplication = ApplicationProvider.getApplicationContext()

    val iParser: IParser = FhirContext.forR4().newJsonParser()
    val qJson =
      fhirApplication.assets.open("sample_patient_registration.json").bufferedReader().use {
        it.readText()
      }
    ...
    val qrJson =
      fhirApplication
        .assets
        .open("sample_registration_questionnaireresponse.json")
        .bufferedReader()
        .use { it.readText() }
    ...
    fhirEngine = mockk()
    coEvery { fhirEngine.load(Patient::class.java, any()) } returns FakePatient.patient1
    coEvery { fhirEngine.save(any()) } answers {}
    coEvery { fhirEngine.update(any()) } answers {}

    fhirApplication.fhirEngine = fhirEngine
    ...
    questionnaireViewModel =
      spyk(QuestionnaireViewModel(ApplicationProvider.getApplicationContext(), config))
  ...
  fun testVerifySavedResource() {
    val sourcePatient = FakePatient.patient1
